...
% load true symbols
load('sym.mat')     
true_bin = importdata(strcat('E:\CHchirp\Code\Config\bin\SF', string(SF), '.txt'))';
...
% lora_set.fft_x = N;
% lora_set.dine = N*upsampling_factor;
% for index = 1:9
%     samples_dechirp = x_1_dnsamp((index-1)*N+1:index*N) .* DC;
%     samples_fft = abs(fft(samples_dechirp,N));
%     subplot(3,3,index);
%     plot(1:N, samples_fft,'');
% end
...
ser = sum(sum(repmat(true_bin,size(demod_sym_stack,1),1) ~= demod_sym_stack)) / prod(size(demod_sym_stack));
disp('******************************************')

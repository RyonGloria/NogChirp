...
float frequency = 435.03125;  // 434.96875MHz ~ 435.09375MHz
...
void loop()
{
    ...
    if (rf95.available())  // Will be set by the interrupt handler when a good message is received
    {
        ...
        if (rf95.recv(buf, &len))
        {
            ...
            Send_Flag = !Send_Flag;
            rf95.setFrequency(frequency);  // Setup frequency of 435.03125MHz
        }else{
            logger.info("Wait for Msg from Controller.");
        }
    }

    // Send a message, Device 3 use frequency of 435.03125MHz
    if (Send_Flag)
    {

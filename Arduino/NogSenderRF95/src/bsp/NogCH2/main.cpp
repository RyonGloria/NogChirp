...
float frequency = 435.0;  // 434.9375MHz ~ 435.0625MHz
...
void setup()
{
    ...
    rf95.setFrequency(437.0);               // Setup ISM frequency  437
...
void loop()
{
    ...
    if (rf95.available())  // Will be set by the interrupt handler when a good message is received
    {
        ...
        { 
            ...
            Send_Flag = !Send_Flag;
            rf95.setFrequency(frequency);  // Setup frequency of 435.0MHz
        }else{
            logger.info("Wait for Msg from Controller.");
        }
    }

    // Send a message, Device 2 use frequency of 435.0MHz
    if (Send_Flag)
    {
        uint8_t data[] = "5a7d6a9b2f3e4180cd0b";

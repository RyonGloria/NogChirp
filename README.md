...
CIC 解码结果：
 ...
 payload1: [810 722 386 614 850 593 126 1022 842 677 347 862 581 551 574 113 806 827 822 85 421 587 421]

 payload2: [810 1010 386 614 850 406 126 1022 367 677 347 284 27 551 1004 1006 49 836 822 85 265 587 421]
...
6. - **问题**：频谱泄露的问题：CIC symbol 切分会导致严重的频谱泄露，我的方案是否可以避免这一问题
   - **答**：
7. - **问题**：互相关性能否用在 SFD 的检测中？
   - **答**：参考 CIC 方法
8. - **问题**：由于信号是含正负方向的，滤波器无法滤掉指定的频率
   - **答**：
9. - **问题**：连续线性变化计算开销大，是否可以从频率角度直接考虑两个窗口的交集
   - **答**：
10. - **问题**：切分的滑动窗口，de-chirp 结果存在峰值偏移的问题
    - **答**：是否可以通过设置置信区间解决该问题
11. - **问题**：
    - **答**： 

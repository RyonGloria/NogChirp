    ...
    if exist(fileDir, "dir")
        ...
        while true
            ...
            for time = 1:round
                ...
                % 解码
                try
                    DebugUtil.exp("", "该次实验开始解码=======================================");
                    CHchirpMultiDecoder = CHchirpMultiDecoder.decode(signalAll);
                    DebugUtil.exp("", "detect发现次数：" + string(CHchirpMultiDecoder.detectCount) ...
                                        + "SFD发现次数：" + string(CHchirpMultiDecoder.detectSFDCount) ...
                                        + "冲突发现成功率");
                    DebugUtil.exp("", "该次实验解码结束=======================================");
                catch
%                     disp("参数：" + argsName ...
%                         + "\n文件:" + fileIndex ...
%                         + "\nwinoff: " + winoff ...
...
%                         + "\n power" + AMP + "出现错误");
                ...
                record{11, countAll} = trueRate;
                record{12, countAll} = recordRateTmp;
                record{13, countAll} = decodeFlag;
                decodeFlag = false;
            ...
            if count >= 40 && abs(trueRateAll/((count-countError)-(round-countErrorTmp)) - (trueRateAll+trueRateNow)/(count-countError)) < 0.01
                ...
                if convergeCount >= 3
                    ...
                    % SNR超过10，实验结束
                    if SNR >= 10
                        DebugUtil.exp("当前冲突数目大于等于14,实验结束");

...
% 读取文件夹下所有采样值文件
fileDir = 'D:\CHchirp_20Nodes\Pyramid_SF' + string(loraSet.sf) ...
    + '_BW' + string(loraSet.bw/1000) ...
    + 'K';
...
%                             signal = signal + noise;
            ...
            record{10, countAll} = trueRateRoundAll;
            record{11, countAll} = channelBinTrueRate;
            record{12, countAll} = preambleChannelNum;
            record{13, countAll} = SRRRecord;
            record{14, countAll} = decodeFlag;
        ...
        % 趋于稳定
        if count >= 40 && abs(trueRateAll/((count-countError)-(round-countErrorTmp)) - (trueRateAll+trueRateNow)/(count-countError)) < 0.01
            convergeCount = convergeCount + 1;
            if convergeCount >= 3
                result{collNum} = (trueRateAll+trueRateNow)/(count-countError);
                save(".\Result\CICCollisionExp1\" + datestr(now, 30) + argsName + ".mat", 'record', "result");
                disp("当前冲突数目结果收敛: " + result{collNum});
                % 正确率低于阈值，结束程序
    ...
    xlabel('冲突数目');
    ylabel('symbol正确率');
    saveas(gcf, ".\Result\CICCollisionExp1\" + datestr(now, 30) + argsName + ".fig");
    saveas(gcf, ".\Result\CICCollisionExp1\" + datestr(now, 30) + argsName + ".png");
    save(".\Result\CICCollisionExp1\" + datestr(now, 30) + argsName + ".mat", "record", "result");
end
